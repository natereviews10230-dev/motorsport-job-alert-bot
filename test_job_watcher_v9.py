...
def test_redbull_preview_link_pattern_accepts_known_posting_shape():
    ...
    fallback = rbr['browser_fallback']
    parser_source = dict(rbr)
    parser_source.update(fallback)
    jobs = parse_link_page_html(parser_source, fallback['url'], page, source_type='browser_link_page')
    assert len(jobs) == 1
...
def test_notify_ntfy_summary_has_no_action_button(monkeypatch):
    monkeypatch.setenv('NTFY_TOPIC', 'test-topic')
    monkeypatch.setenv('NTFY_SERVER', 'https://ntfy.sh')
...
def test_smartrecruiters_vcarb_buyer_is_fetched():
    payload = {
        'offset': 0,
        'limit': 100,
        'totalFound': 1,
        'content': [{
            'id': '744000125844743',
            'name': 'VCARB F1 Team - Buyer (Composite)',
            'releasedDate': '2026-08-21T10:00:00Z',
            'company': {'name': 'Red Bull'},
            'location': {'city': 'Faenza', 'region': 'Emilia-Romagna', 'country': 'it'},
            'department': {'label': 'Purchasing'},
            'function': {'label': 'Supply Chain'},
    ...
    jobs = fetch_smartrecruiters({
        ...
        'company': 'Visa Cash App Racing Bulls F1 Team',
        'company_identifier': 'RedBull',
        'query': 'VCARB F1 Team',
    }, session, 10)
    assert len(jobs) == 1
    assert jobs[0].title == 'VCARB F1 Team - Buyer (Composite)'
    assert '744000125844743' in jobs[0].url
    assert 'Purchasing' in jobs[0].description


def test_cadillac_link_page_paginates_until_no_new_jobs():
    page0 = """<html><body><a href='/en/jobs/a1-technical-buyer'>Technical Buyer</a></body></html>"""
    page1 = """<html><body><a href='/en/jobs/a2-finance-analyst'>Finance Analyst (Operations & Racing)</a></body></html>"""
    page2 = """<html><body></body></html>"""
    session = FakeSession([
        FakeResponse(text=page0, content_type='text/html'),
        FakeResponse(text=page1, content_type='text/html'),
        FakeResponse(text=page2, content_type='text/html'),
    ])
    jobs = fetch_link_page({
        'name': 'Cadillac Formula 1 Team',
        'url': 'https://opportunities.cadillacf1team.com/en/jobs',
        'include_url_regex': r'opportunities\.cadillacf1team\.com/(?:en/)?jobs/.+',
        'exclude_url_regex': r'/jobs/?(?:$|[?#])',
        'page_param': 'pageIndex',
        'page_start': 0,
        'max_pages': 10,
        'stop_when_no_new_jobs': True,
        'respect_robots': False,
    }, session, 10, 'test')
    assert [job.title for job in jobs] == [
        'Technical Buyer', 'Finance Analyst (Operations & Racing)'
    ]


def test_live_config_covers_buyer_procurement_and_cnc_probe():
    cfg = _live_config()
    global_filters = cfg['filters']
    assert 'buyer' in global_filters['title_any']
    assert 'procurement' in global_filters['title_any']

    rbr = next(s for s in cfg['sources'] if s['name'] == 'Red Bull Racing & Technology')
    merged = merge_source_title_filters(global_filters, rbr)
    cnc_job = Job(
        rbr['name'], 'oracle_hcm', '10022', 'CNC Machinist (Night Shift)',
        rbr['company'], 'Milton Keynes, UK',
        'https://careers.redbullracing.com/en/sites/CX_2/job/10022'
    )
    assert matches_filters(cnc_job, merged)


def test_live_config_cadillac_finance_and_buyer_titles_match():
    cfg = _live_config()
    global_filters = cfg['filters']
    finance = Job('Cadillac Formula 1 Team', 'link_page', '1',
                  'Finance Analyst (Operations & Racing)', 'Cadillac Formula 1 Team',
                  'Silverstone, UK', 'https://example.com/1')
    buyer = Job('Cadillac Formula 1 Team', 'link_page', '2',
                'Technical Buyer', 'Cadillac Formula 1 Team',
                'Silverstone, UK', 'https://example.com/2')
    assert matches_filters(finance, global_filters)
    assert matches_filters(buyer, global_filters)
