...
def fetch_smartrecruiters(
    ...
    """Fetch active public SmartRecruiters postings for one company.

    SmartRecruiters exposes a public Posting API, so this avoids scraping
    LinkedIn or relying on a JavaScript-rendered careers landing page.
    """
    company_identifier = source["company_identifier"].strip()
    ...
    api_base = (
        ...
        f"{quote(company_identifier, safe='')}/postings"
    )
    ...
    for _ in range(max_pages):
        ...
        payload = request_json(
            session,
            api_base + "?" + urlencode(params),
            timeout,
            headers={"Accept-Language": source.get("language", "en")},
        )
        ...
        for item in postings:
            if not isinstance(item, dict):
                continue
            external_id = clean_text(pick(item, "id", "uuid", "refNumber"))
            title = clean_text(pick(item, "name", "title"))
            location_parts = [
                clean_text(pick(item, "location.city")),
                clean_text(pick(item, "location.region")),
                clean_text(pick(item, "location.country")),
            ]
            location = ", ".join(part for part in location_parts if part)
            ui_url = clean_text(
                pick(item, "postingUrl", "jobAdUrl", "applyUrl", "url")
            )
            if not ui_url and external_id:
                suffix = slugify(title)
                ui_url = (
                    f"https://jobs.smartrecruiters.com/{company_identifier}/"
                    f"{external_id}" + (f"-{suffix}" if suffix else "")
                )
            description = " ".join(
                filter(
                    None,
                    [
                        labeled_text("Department", pick(item, "department.label")),
                        labeled_text("Function", pick(item, "function.label")),
                        labeled_text("Employment Type", pick(item, "typeOfEmployment.label")),
                        labeled_text("Experience Level", pick(item, "experienceLevel.label")),
                        labeled_text("Company", pick(item, "company.name")),
                    ],
                )
            )
            jobs.append(
                Job(
                    ...
                    external_id=external_id or stable_id(ui_url),
                    title=title,
                    company=source.get("company", clean_text(pick(item, "company.name")) or source["name"]),
                    location=location,
                    url=ui_url,
                    description=description,
                    posted_at=clean_text(pick(item, "releasedDate", "createdOn")),
                )
            )

        offset += len(postings)
        total = int(payload.get("totalFound", offset)) if isinstance(payload, dict) else offset
...
def fetch_link_page(
    ...
    base_urls = as_list(source.get("urls") or source.get("url"))
    jobs: list[Job] = []

    page_param = str(source.get("page_param", "")).strip()
    page_start = int(source.get("page_start", 0))
    max_pages = max(int(source.get("max_pages", 1)), 1)

    page_urls: list[str] = []
    for base_url_value in base_urls:
        ...
        for page_index in range(page_start, page_start + max_pages):
            query_map = {key: list(values) for key, values in original_query.items()}
            query_map[page_param] = [str(page_index)]
            flattened: list[tuple[str, str]] = []
            for key, values in query_map.items():
                for value in values:
                    flattened.append((key, value))
            page_urls.append(
                parsed._replace(query=urlencode(flattened)).geturl()
            )
    ...
    for page_number, page_url in enumerate(page_urls):
        ...
        response.raise_for_status()
        page_jobs = parse_link_page_html(source, page_url, response.text)
        jobs.extend(page_jobs)
        jobs = dedupe_jobs(jobs)

        if page_param and source.get("stop_when_no_new_jobs", True):
            if page_number > 0 and len(jobs) == previous_total:
                break
...
def fetch_oracle_hcm_with_browser_fallback(
    source: dict[str, Any],
    session: requests.Session,
    timeout: int,
    user_agent: str,
) -> list[Job]:
    """Prefer Oracle's public requisition API, with rendered-page fallback.

    This is useful for Oracle Candidate Experience sites that occasionally
    change their client-side listing markup while keeping the requisition API.
    """
    fallback = source.get("browser_fallback")
    try:
        jobs = fetch_oracle_hcm(source, session, timeout)
        if jobs or not isinstance(fallback, dict):
            return jobs
        logging.warning(
            "%s: Oracle API returned 0 jobs; trying browser fallback",
            source.get("name", "Oracle source"),
        )
    except Exception as exc:
        if not isinstance(fallback, dict):
            raise
        logging.warning(
            "%s: Oracle API failed (%s); trying browser fallback",
            source.get("name", "Oracle source"),
            exc,
        )
    ...
    fallback_source["type"] = "browser_link_page"
    fallback_source.pop("browser_fallback", None)
    return fetch_browser_link_page(
        fallback_source, session, timeout, user_agent
    )
...
def fetch_source(
    ...
    if source_type == "oracle_hcm":
        return fetch_oracle_hcm_with_browser_fallback(
            source, session, timeout, user_agent
        )
    if source_type == "smartrecruiters":
        return fetch_smartrecruiters(source, session, timeout)
    if source_type == "html":
...
def notify_ntfy_summary(
    ...
    *,
    ...
    # V8: summary notifications intentionally contain no Click/Actions header.
    # The notification body still lists every matching role, while
    # current_matches.md remains available in the repository for manual review.
...
def run_cycle(
    ...
    *,
    ...
    # current match and highlights jobs first detected in this run. ntfy itself
    # stays text-only; the dashboard is available only when the user opens GitHub.
