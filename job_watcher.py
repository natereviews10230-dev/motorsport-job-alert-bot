...
def enrich_job_from_detail_html(job: Job, page_html: str, source: dict[str, Any]) -> Job:
    """Recover a real title/location from a job detail page.

    Dynamic careers pages sometimes expose a job link whose listing-card anchor text is
    only "Learn more" or whose location is omitted. JobPosting JSON-LD is the preferred
    source; visible H1/location text is used as a fallback.
    """
    soup = BeautifulSoup(page_html, "html.parser")
    ...
    if normalize(title) in generic_titles or len(title) > 240:
        heading = soup.select_one("h1") or soup.select_one("[class*='job'][class*='title']")
        if heading:
    ...
    if not location:
        ...
        if not location and source.get("location_regex"):
            ...
            match = re.search(source["location_regex"], page_text, re.I)
            if match:
                location = clean_text(match.groupdict().get("location") or match.group(1))
...
def filter_failure_reasons(job: Job, filters: dict[str, Any], prefix: str = "") -> list[str]:
    ...
    if title_any and not contains_any(job.title, title_any):
        reasons.append(label + "title did not match allowed title families")
