...
def fetch_smartrecruiters(
    ...
    """Fetch public postings from the SmartRecruiters Posting API.

    SmartRecruiters exposes published company postings without authentication.
    This is more reliable than scraping a branded Attrax careers front end and
    supports complete pagination through limit/offset.
    """
    company_identifier = source["company_identifier"].strip()
    base_url = source.get("feed_url") or (
    ...
    max_pages = max(int(source.get("max_pages", 50)), 1)
    fetch_details = bool(source.get("fetch_details", False))
    jobs: list[Job] = []
    offset = 0

    for _ in range(max_pages):
        query = urlencode({"destination": "PUBLIC", "limit": page_size, "offset": offset})
        payload = request_json(session, f"{base_url}?{query}", timeout)
        if not isinstance(payload, dict):
            break
        items = as_list(payload.get("content"))
        if not items:
            break
        ...
        for item in items:
            if not isinstance(item, dict):
                continue
            posting_id = clean_text(pick(item, "id", "uuid"))
            title = clean_text(pick(item, "name", "title"))
            location_obj = item.get("location") if isinstance(item.get("location"), dict) else {}
            location = clean_text(location_obj.get("fullLocation"))
            if not location:
                location = ", ".join(
                    x for x in [
                        clean_text(location_obj.get("city")),
                        clean_text(location_obj.get("region")),
            ...
            if fetch_details and posting_id:
                try:
                    detail = request_json(
                        session,
                        f"{base_url}/{quote(posting_id, safe='')}",
                        timeout,
                    )
                    ...
                    logging.warning(
                        "Could not fetch SmartRecruiters detail %s: %s", posting_id, exc
                    )
                    detail = item

            sections = pick(detail, "jobAd.sections", default={})
            ...
            metadata = [
                labeled_text("Department", pick(item, "department.label", "department")),
                labeled_text("Function", pick(item, "function.label", "function")),
                labeled_text("Employment", pick(item, "typeOfEmployment.label", "typeOfEmployment")),
                labeled_text("Experience", pick(item, "experienceLevel.label", "experienceLevel")),
            ]
            description = " ".join(filter(None, [*metadata, *description_parts]))

            link = clean_text(pick(detail, "postingUrl", "applyUrl"))
            if not link and posting_id:
                link = (
                    f"https://jobs.smartrecruiters.com/"
                    f"{quote(company_identifier, safe='')}/{quote(posting_id, safe='')}"
                )
            ...
            jobs.append(
                Job(
                    ...
                    external_id=posting_id or stable_id(link or title),
                    title=title,
                    company=source.get("company", clean_text(pick(item, "company.name")) or source["name"]),
                    location=location,
                    url=link,
                    description=description,
                    posted_at=clean_text(pick(item, "releasedDate", "released_date")),
                )
            )

        total = int(payload.get("totalFound") or len(items))
        returned_limit = int(payload.get("limit") or page_size)
        returned_offset = int(payload.get("offset") or offset)
        next_offset = returned_offset + max(returned_limit, len(items))
        if next_offset >= total or len(items) < page_size:
            break
        offset = next_offset
